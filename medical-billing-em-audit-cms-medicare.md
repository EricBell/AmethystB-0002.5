...
This step matters. Weak reviews often go wrong by counting every clinical sentence as if it supports the code. Routine medication refills without active assessment often belong in the "Mentioned but not meaningfully managed" bucket rather than the active problem bucket.
...
State whether the documentation satisfies the reporting requirements for that add-on code and explain why or why not. Identify the specific standard you are measuring the add-on code against so the reasoning is explicit.
...
### What to look for
The note should support the specific standard for the add-on code being billed. For Medicare complexity-based add-on codes, look for documentation that actually shows the required ongoing care relationship, longitudinal management of a complex or serious condition, or inherent visit complexity tied to that kind of condition.
